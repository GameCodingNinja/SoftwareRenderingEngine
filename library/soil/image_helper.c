...
void SwapRedandBlue( unsigned char *pImage, int width, int height, int channels )
{
    int y, i, posY, posX, scanLineSizeInBytes;
    unsigned char temp;

    // Get the size of the data
    scanLineSizeInBytes = (width * channels) * sizeof( unsigned char );
    ...
    for( y = 0; y < height; ++y )
    {
        ...
        {            
            ...
            posX = posY + (i * channels);

            // Swap red and blue
            temp = pImage[ posX + 0 ];
            pImage[ posX + 0 ] = pImage[ posX + 2 ];
            pImage[ posX + 2 ] = temp;
        }
    }
}   // SwapRedandGreen
